...
class dogImage(object):
    ...
    def calcObj(self):
        #self.obj = predictImage2(self.image_data)
        #return
...
##                    #print(score)
    ...
    def mutateB(self):
        ...
        hue = numpy.array([(random.random()*2),\
...
def getRandomRectEnd(image, height, width):
    ...
    bottomx = topx+width
    bottomy = topy+height
    if 0<=targetDog[topy][topx][0]<=5 and 250<=targetDog[topy][topx][1]<=255 and\
       0<=targetDog[topy][topx][2]<=5:
...
targetDog = Image.open("ace_green_2.jpg")
targetDog = targetDog.resize((128,128), Image.BILINEAR)

targetDog = keras.preprocessing.image.img_to_array(targetDog)

origfile = sys.argv[1]

im1 = Image.open(origfile)
im2 = im1.resize((128,128), Image.BILINEAR)
